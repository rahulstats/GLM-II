...
idx1 <- which(all_triads[1,] == 1 & all_triads[2,] == 2 & all_triads[3,] == 3)
idx2 <- which(all_triads[1,] == 1 & all_triads[2,] == 4 & all_triads[3,] == 5)

true_triads <- c(idx1, idx2) 
n_true <- length(true_triads) 

cat("True Triad Indices mapping to (1,2,3) and (1,4,5):", true_triads, "\n")

# ORIGINAL DEFINITION: nu represents the TOTAL expected margin
nu = as.vector(mu_par(K)/10 + a*CK[,true_triads[1]] + b*CK[,true_triads[2]])
