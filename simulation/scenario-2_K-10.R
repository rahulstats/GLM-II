...
a = 1; b = -1; c= 1; d =-1
nu = mu_par(K) + a*C_mat(K)[,16] + b*C_mat(K)[,50] + c*C_mat(K)[,66]
############################################################
rep_sim=1000
...
for (ai in 1:rep_sim) {
  ...
  {n_triad[ai] = NCOL(Mat2); true_model_select[ai] = sum(c(tick3_col,tick2_col)%in%16, c(tick3_col,tick2_col)%in%50, 
                                                         ...
                                                         {n_triad[ai] = NCOL(Mat1); true_model_select[ai] = 
                                                           sum(c(tick3_col,tick2_col,tick1_col)%in%16, c(tick3_col,tick2_col,tick1_col)%in%50, c(tick3_col,tick2_col,tick1_col)%in%66); triad_tick[ai]= 1}
  ############################################
  ## outcome LASSO #####
  ######################
  LC = L_Mat%*%CK
  ...
  true_model_select_lasso[ai] = sum(lasso_select%in%16, lasso_select%in%50, lasso_select%in%66)
  ############################################
  ...
  n_triad_fstep[ai]=length(fm.step_select); true_model_select_fstep[ai]=sum(fm.step_select%in%16, fm.step_select%in%50, fm.step_select%in%66)
  ##################################################################################
  ...
  true_model_select_sts[ai]=sum(sts_select%in%16, sts_select%in%50, sts_select%in%66)
}
### output
