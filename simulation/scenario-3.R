...
a = 1; b = -1; c= 1; d =-1
nu = mu_par(K) + a*C_mat(K)[,1] + b*C_mat(K)[,2] 
nu_cyclic = a*C_mat(K)[,1] + b*C_mat(K)[,2] 
############################################################
rep_sim=100
...
for (ai in 1:rep_sim) {
  ...
  {n_triad[ai] = NCOL(Mat3); true_model_select[ai] = sum(isTRUE(qr(CK[,tick3_col])$rank== qr(cbind(CK[,tick3_col],nu_cyclic))$rank))
    #sum(in.span(CK[,tick3_col],nu_cyclic));
  triad_tick[ai] = 3} else if (model_002_pval>0.05)
  {n_triad[ai] = NCOL(Mat2); 
  true_model_select[ai] = sum(isTRUE(qr(CK[,c(tick3_col,tick2_col)])$rank== qr(cbind(CK[,c(tick3_col,tick2_col)],nu_cyclic))$rank))
    #sum(in.span(CK[,c(tick3_col,tick2_col)],nu_cyclic));
  triad_tick[ai] = 2} else
  {n_triad[ai] = NCOL(Mat1); 
  true_model_select[ai] = sum(isTRUE(qr(CK[,c(tick3_col,tick2_col,tick1_col)])$rank== qr(cbind(CK[,c(tick3_col,tick2_col,tick1_col)],nu_cyclic))$rank))
    #sum(in.span(CK[,c(tick3_col,tick2_col,tick1_col)],nu_cyclic));
  triad_tick[ai]= 1}
  ############################################
  ## outcome LASSO #####
  ######################
  ...
  true_model_select_lasso[ai] = sum(isTRUE(qr(CK[,lasso_select])$rank== qr(cbind(CK[,lasso_select],nu_cyclic))$rank))
  ############################################
  ...
  true_model_select_fstep[ai]= sum(isTRUE(qr(CK[,fm.step_select])$rank== qr(cbind(CK[,fm.step_select],nu_cyclic))$rank))
  ##################################################################################
  ...
  true_model_select_sts[ai]= sum(isTRUE(qr(CK[,sts_select])$rank== qr(cbind(CK[,sts_select],nu_cyclic))$rank))
}
### output
fst_out= cbind(n_triad, true_model_select)
fst_out_true = fst_out[fst_out[,2]==1,] # true model selected part
nrow(fst_out_true)/rep_sim; mean(fst_out[,1])
## lasso
lasso_out = cbind(n_triad_lasso, true_model_select_lasso)
lasso_out_true = lasso_out[lasso_out[,2]==1,] # true model selected part
nrow(lasso_out_true)/rep_sim; mean(lasso_out[,1])
## forward stepwise
fstep_out = cbind(n_triad_fstep, true_model_select_fstep)
fstep_out_true = fstep_out[fstep_out[,2]==1,] # true model selected part
nrow(fstep_out_true)/rep_sim; mean(fstep_out[,1])
## sts
sts_out = cbind(n_triad_sts, true_model_select_sts)
sts_out_true = sts_out[sts_out[,2]==1,] # true model selected part
