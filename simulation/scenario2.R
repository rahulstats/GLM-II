...
#-----------------------------#
# True Model: 1*(1,4,5) - 1*(2,5,6) + 1*(3,4,6)
a = 1; b = -1; c = 1

# Dynamically find the exact column indices
all_triads <- combn(K, 3)
idx1 <- which(all_triads[1,] == 1 & all_triads[2,] == 4 & all_triads[3,] == 5)
idx2 <- which(all_triads[1,] == 2 & all_triads[2,] == 5 & all_triads[3,] == 6)
idx3 <- which(all_triads[1,] == 3 & all_triads[2,] == 4 & all_triads[3,] == 6)

true_triads <- c(idx1, idx2, idx3) 
n_true <- length(true_triads) 

cat("True Triad Indices mapping to (1,4,5), (2,5,6), (3,4,6):", true_triads, "\n")

nu = as.vector(mu_par(K)/10 + a*CK[,true_triads[1]] + b*CK[,true_triads[2]] + c*CK[,true_triads[3]])
