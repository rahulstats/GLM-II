...
a = 1; b = -1; c= 1; d =-1
nu = mu_par(K) + a*C_mat(K)[,8] + b*C_mat(K)[,16] + c*C_mat(K)[,18]# + d*C_mat(K)[,10]
############################################################
rep_sim=1000
...
for (ai in 1:rep_sim) {
  ...
  {n_triad[ai] = NCOL(Mat3); true_model_select[ai] = sum(tick3_col%in%8, tick3_col%in%16, tick3_col%in%18);
  triad_tick[ai] = 3} else if (model_002_pval>0.05)
  {n_triad[ai] = NCOL(Mat2); true_model_select[ai] = sum(c(tick3_col,tick2_col)%in%8, c(tick3_col,tick2_col)%in%16, 
                                                         c(tick3_col,tick2_col)%in%18);triad_tick[ai] = 2} else
                                                         {n_triad[ai] = NCOL(Mat1); true_model_select[ai] = 
                                                           sum(c(tick3_col,tick2_col,tick1_col)%in%8, c(tick3_col,tick2_col,tick1_col)%in%16, c(tick3_col,tick2_col,tick1_col)%in%18); triad_tick[ai]= 1}
  ############################################
  ## outcome LASSO #####
  ######################
  LC = L_Mat%*%CK
  ...
  true_model_select_lasso[ai] = sum(lasso_select%in%8, lasso_select%in%16, lasso_select%in%18)
  ############################################
  ...
  n_triad_fstep[ai]=length(fm.step_select); true_model_select_fstep[ai]=sum(fm.step_select%in%8, fm.step_select%in%16, fm.step_select%in%18)
  ##################################################################################
  ...
  true_model_select_sts[ai]=sum(sts_select%in%8, sts_select%in%16, sts_select%in%18)
}
### output
fst_out= cbind(n_triad, true_model_select)
fst_out_true = fst_out[fst_out[,2]==3,] # true model selected part
nrow(fst_out_true)/rep_sim; mean(fst_out[,1])
## lasso
lasso_out = cbind(n_triad_lasso, true_model_select_lasso)
lasso_out_true = lasso_out[lasso_out[,2]==3,] # true model selected part
nrow(lasso_out_true)/rep_sim; mean(lasso_out[,1])
## forward stepwise
fstep_out = cbind(n_triad_fstep, true_model_select_fstep)
fstep_out_true = fstep_out[fstep_out[,2]==3,] # true model selected part
nrow(fstep_out_true)/rep_sim; mean(fstep_out[,1])
## sts
sts_out = cbind(n_triad_sts, true_model_select_sts)
sts_out_true = sts_out[sts_out[,2]==3,] # true model selected part
nrow(sts_out_true)/rep_sim; mean(sts_out[,1])
##
matrix(c(nrow(fstep_out_true)/rep_sim, mean(fstep_out[,1])/3,
         nrow(lasso_out_true)/rep_sim, mean(lasso_out[,1])/3,
         nrow(fst_out_true)/rep_sim, mean(fst_out[,1])/3,
         nrow(sts_out_true)/rep_sim, mean(sts_out[,1])/3), nrow = 4, byrow = T)
